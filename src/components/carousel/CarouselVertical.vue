...
<div :class="[prefix, arrow]" @mouseenter="stopTimer" @mouseleave="startTimer">
  ...
  <ul :class="[`${prefix}_dots`, dots, {radiusDot}]">
    <li v-for="i in count" :key="i" :class="{active: i === curIndex + 1}" @click="toIndex(i, $event)" @mouseover="toIndex(i, $event)"></li>
  </ul>
</div>
</template>
...
export default {
  ...
  props: {
    ...
    autoplaySpeed: {
      type: N,
      default: 5000 // autoplaySpeed 60秒，transform 40秒，差额20秒就是单页面停留时间，40秒为滚动时间----修改轮播
    },
    dots: {
      default: 'inside',
      validator(v) {
  ...
  methods: {
    ...
    getListStyle(hasAnimate) { // 获取列表样式
      const style = {
        height: parseSize(this.height * this.len),
        transform: `translateY(-${this.curIndex * this.height}px)`
        // animation: `gun 8s linear `
      }
      return hasAnimate ? {
        ...style,
        transition: `transform 45s ${this.easing} 1s` // autoplaySpeed 60秒，transform 40秒，差额20秒就是单页面停留时间，40秒为滚动时间----修改轮播
